...
async fn fast_validate_proxy(&self, proxy: &ProxyInfo) -> Result<bool, Box<dyn Error>> {
    ...
    let proxy_client = match Client::builder()
        ...
        .timeout(Duration::from_secs_f64(self.timeout_duration * 0.5))  // Shorter initial timeout
    ...
    // Fast initial check before doing multiple rounds
    match timeout(
        Duration::from_secs_f64(self.timeout_duration * 0.5),
        proxy_client.get(test_url).send()
    ).await {
        Ok(Ok(response)) => {
            if response.status().is_success() {
                // If this initial check passes, do the full validation
                let mut successful_rounds = 1;
                
                for _ in 1..self.validation_rounds {
                    // Only test with additional URLs if first one worked
                    let next_test_url = self.get_next_test_url();
                    
                    match timeout(
                        Duration::from_secs_f64(self.timeout_duration),
                        proxy_client.get(next_test_url).send()
                    ).await {
                        Ok(Ok(response)) => {
                            if response.status().is_success() {
                                successful_rounds += 1;
                ...
                let is_valid = match self.validation_rounds {
                    1 => true,  // Already validated in first check
